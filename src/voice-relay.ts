...
export async function createTelnyxCall(
  ...
  if (dynamicVariables && Object.keys(dynamicVariables).length > 0) {
    payload.client_state = Buffer.from(JSON.stringify(dynamicVariables)).toString("base64");
  }
  ...
  if (!resp.ok) {
    throw new Error(`Telnyx call create failed (${resp.status}): ${bodyText}`);
  }
...
async function handleOutboundStart(request: Request): Promise<Response> {
  ...
  try {
    ...
    to = (payload.to || "").trim();
    if (payload.dynamic_variables && typeof payload.dynamic_variables === "object") {
      dynamicVariables = Object.entries(payload.dynamic_variables).reduce<Record<string, string>>(
        (acc, [k, v]) => {
          acc[k] = typeof v === "string" ? v : JSON.stringify(v);
          return acc;
        },
...
async function handleTelnyxWebhook(request: Request): Promise<Response> {
  ...
  console.log(`[Telnyx webhook] ${eventType} call_control_id=${callControlId} call_leg_id=${callLegId}`);
  return text("ok");
}
...
function printStartup(prefix = ""): void {
  ...
  console.log(`${p}Telnyx connection id: ${TELNYX_CONNECTION_ID ? "set" : "not set"}`);
  console.log(`${p}Outbound auth token: ${OUTBOUND_AUTH_TOKEN ? "set" : "not set"}`);
}
...
export function startVoiceRelay(prefix = "[voice]"): Bun.Server {
  printStartup(prefix);
  ...
  return Bun.serve({
    ...
    async fetch(request) {
      ...
      if (request.method === "POST" && url.pathname === "/telnyx/outbound/start") {
        return handleOutboundStart(request);
      }

      if (request.method === "POST" && url.pathname === "/telnyx/webhook") {
